...
Pertama kita download file terlebih dahulu lalu mengecek file tersebut memiliki format apa ```file shark2.pcapng```
Mendapatkan output bahwa pcapng

Kita buka file menggunakan wireshark dengan cara ``` File >> Open >> shark2.pcapng```
Setelah membuka kita dapat pergi ke ```File >> Export Object >> HTTP``` ( untuk melihat objek dengan mudah seperti flag, text dll)

Atau kita juga bisa menggunakan command pada linux kita dengan perintah ```strings shark2.pcapng | grep "pico"```
Dan muncul banyak output ialah flag
```
...
```
Namun jangan sampai terdistraksi, tidak mungkin kita di suruh mencari satu persatu dari flag tsb.

Pada saat saya melihat ___HTTP Object list___ menemukan source ip berasal dari ___18.217.1.57___. Jadi kita akan mencoba tracking

Pada no 57 saya menemukan petunjuk ```The official red's shrimp and herring website is still under construction. Please check back later```
kata kata pada ```Please check back later``` membuat saya ingin mengecek pada paket yang memiliki seperti redshrimp atau herring, maka :

Menggunakan filter 
```dns && ip.dst == 18.217.1.57``` 
dns : untuk memfilter protocol yang di gunakan pada dns saja
&& : menggabungkan output dns dan ip.dst
ip.dst == 18.217.1.57 : untuk memfilter destinasi dari ip tsb

Saya menemukan kode yang dapat dirangkai pada kata sebelum ___.reddshrimpandherring.com___ seperti :
```
cGljb0NU
RntkbnNf
M3hmMWxf
...
fQ==
```
Yang dimana kode ini mirip seperti encode base64, jadi saya rangkai menjadi ```cGljb0NURntkbnNfM3hmMWxfZnR3X2RLYWRiZWVmfQ==```

Dan medecode menggunakan perintah ```echo cGljb0NURntkbnNfM3hmMWxfZnR3X2RLYWRiZWVmfQ== | base 64 -d ```
Mendapatkan flag : ```picoCTF{dns_3xf1l_ftw_...}```
